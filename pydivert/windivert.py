...
class WinDivert(object):
    ...
    def recv(self, bufsize=40 + 0xFFFF):
        ...
        # Copy only the received bytes so the 64KB recv buffer can be freed
        # immediately instead of being held alive by a memoryview slice.
        trimmed = bytearray(packet[:recv_len.value])
        pkt = Packet(
            memoryview(trimmed),
            (address.Data.Network.IfIdx, address.Data.Network.SubIfIdx),
            Direction(not address.Outbound)
        )
        # Preserve the full WinDivert 2.x address for send()
        pkt._wd_addr_raw = address
        return pkt
    ...
    def send(self, packet, recalculate_checksum=True):
        ...
        # WinDivert 2.x: pSendLen before pAddr (order changed from 1.x)
        windivert_dll.WinDivertSend(self._handle, buff, len(packet.raw), byref(send_len),
