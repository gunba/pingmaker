...
class Packet(object):
    ...
    def wd_addr(self):
        """
        Gets the WinDivert address structure for send().

        If a raw WinDivert 2.x address was preserved from recv(), return it
        directly so all flags (Loopback, Outbound, checksums, etc.) are
        kept intact.  Otherwise fall back to reconstructing from interface
        and direction (legacy path).
        """
        if hasattr(self, '_wd_addr_raw') and self._wd_addr_raw is not None:
            return self._wd_addr_raw
        address = windivert_dll.WinDivertAddress()
        address.Data.Network.IfIdx, address.Data.Network.SubIfIdx = self.interface
