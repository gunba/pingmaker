...
From source: `python main.py` (requires Windows 10/11, admin).
...
3. **Multi-character support:** Add character names to automatically track entity IDs via nickname bindings. This ensures only your packets are modified in group content where multiple players' skill packets are coalesced into single TCP segments.

4. **Per-skill overrides:** Click "Skills & Attack Speed" to expand the skills panel, where you can set individual combat speed multipliers or disable modification for specific skills.

5. **Varint cap:** Characters at <=164% combat speed (usually sub-3k GS, no scroll) are capped at 164% due to 2-byte encoding (the packet does not have space for a larger number). Use the **Break Packet** checkbox on no-cooldown skills to bypass this — don't use it on skills with cooldowns, as it breaks cooldown tracking.
